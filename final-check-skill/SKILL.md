...
name: final-check
description: >
  You MUST run this skill as the FINAL step before delivering any project output to the user.
  This is the OUTPUT gate — nothing may be handed to the user without passing final-check.
  Triggers include: preparing to announce project completion, about to provide final file links,
  all implementation tasks appear finished, every todo item is marked completed.
  This is a HARD-GATE — no project output may be delivered without passing final-check.
---
...
1. **Round 1 — Runnable** — verify the code actually compiles, runs, and has no missing dependencies or obvious setup problems. See `skills/final-check/references/round1-runnability.md`.
2. **Round 2 — Quality** — verify no logical errors, unsafe patterns, leftover debug code, or stylistic problems. See `skills/final-check/references/round2-quality.md`.
3. **Round 3 — Matches the Request** — verify the output actually does what the user asked for, not something adjacent. See `skills/final-check/references/round3-requirement-match.md`.
4. **Round 4 — Delivery Ready** — verify the final output is clean, complete, and ready to hand to the user. See `skills/final-check/references/round4-delivery.md`.
